...
- a valid [list storage location](./list-storage-location) (denoting chainId, listRecords contract address, and slot)

- an [account metadata record](./account-metadata) specifying a tokenId as a user's primary list

- a [list metadata record](./list-metadata) in the listRecords contract for 'user' role for the slot

- a [list record](./list-records) with a specified slot that matches the list metadata record and the list storage location.  This list record must have been created on the list records contract and chain specified in the list storage location

![Active List Operation](./public/active-list-op.png)
