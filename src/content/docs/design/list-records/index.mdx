...
`data` should always be 20-bytes for record type `1`.

If any of the above conditions are not met, the `ListRecord` is considered invalid and should not be processed.

## Encoding

Onchain, list records are packed into byte arrays with the **version** and **record type** prepended to the **data** to form an array with `2 + data.length` bytes.
...
// as `bytes` and do not store list records directly
