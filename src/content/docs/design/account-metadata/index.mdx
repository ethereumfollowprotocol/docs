...
Every Ethereum account (address) automatically is an EFP account. Users may follow any other address and any address can mint an EFP List NFT.

It useful to store EFP-related metadata that is specific to an account. This is called **EFP Account Metadata**.

Every Ethereum account may store key-value data in the `EFPAccountMetadata` contract specific to their account.
...
There is only one global key currently defined.
