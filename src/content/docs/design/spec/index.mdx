...
**Followers** is the total number of EFP List NFTs that have at least one Address Record with a specific Ethereum address, with the following additional conditions:

- Exclude records with either the "block" or "mute" tags, even if the records have other tags.
- Multiple EFP List NFTs with an identical User are counted as a single follower.

**Following** is the total number of Entry Records of a user's EFP List NFT, with the following additional conditions:
